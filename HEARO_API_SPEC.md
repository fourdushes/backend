...
| [진료 시작] | 수락된 요청의 채팅방과 아카이브 생성 | POST | `/api/medical-treatment/ward/requests/{requestId}/start` | 양준형 | 완료 | Yes |
...
| [진료 요청 수락] | 기관 사용자가 진료 요청 수락 | POST | `/api/medical-treatment/institution/requests/{requestId}/accept` | 양준형 | 완료 | Yes |
...
| [진료 종료] | 진료 종료 및 전체 채팅 내용 아카이브 저장 | POST | `/api/medical-treatment/ward/chat-rooms/{chatRoomId}/complete` | 양준형 | 완료 | Yes |
...
서비스의 `IllegalArgumentException`과 `IllegalStateException`은 HTTP 400, `AuthenticationException`은 HTTP 401로 변환된다. 별도 처리되지 않은 예외는 코드만으로 응답 형식을 확정할 수 없다.
...
| Auth(인증) | No |
| Description | 요청 Body의 Refresh Token과 Redis 저장값을 검증해 토큰 쌍을 재발급한다. |
...
`data.totalCount`, `data.careList[] { wardUserId, guardUserId, careState, createdAt, updatedAt }`. `careState`: `APPROVED`, `PENDING`, `REJECTED`.
...
`data.totalCount`, `data.guardSearchList[] { guardUserId, guardUserName, userType, mainGuardUser }`.
...
`data.totalCount`, `data.wardSearchList[] { wardUserId, wardUserName, userType, mainGuardUser }`.
...
`data[]`는 `MedicalRequestResponse` 배열. 잘못된 사용자 유형은 HTTP 400.
...
| Auth(인증) | Yes (WARD) |
| Description | 수락된 요청을 진료 중으로 전환하고 Archive·ChatRoom·첫 메시지를 만든다. |
...
`data { chatRoomId, archiveId }`.

#### Error Codes

HTTP 400: 요청 없음, 잘못된 사용자 유형, `요청을 보낸 피보호자만 진료를 시작할 수 있습니다.`, `이미 시작된 진료입니다.`, `현재 상태에서는 요청을 처리할 수 없습니다. 현재 상태: {status}` 및 공통 인증 오류.
...
20번과 같은 요청·응답 구조이며 Path만 기관 경로다. 요청 대상 기관 사용자 또는 요청 피보호자만 조회할 수 있다.
...
| Auth(인증) | Yes (INSTITUTIONS) |
| Description | 요청 대상 기관 사용자가 REQUESTED 요청을 ACCEPTED로 변경한다. |

#### Request / Response

Path `requestId`(number, 필수), 응답 `data`는 `MedicalRequestResponse`.

#### Error Codes

HTTP 400: 요청 없음, 잘못된 사용자 유형, `요청 대상 기관 사용자만 처리할 수 있습니다.`, 현재 상태 처리 불가 및 공통 인증 오류.

### 27) [진료 요청 거절] `/api/medical-treatment/institution/requests/{requestId}/reject`

24번과 같은 구조이며 요청 상태를 `REJECTED`로 변경한다.
...
| Auth(인증) | Yes (WARD) |
| Description | 채팅 내용을 Archive에 합쳐 저장하고 진료·채팅방을 완료 처리한다. |
...
HTTP 400: 잘못된 사용자 유형, 채팅방 없음, `해당 진료의 피보호자만 진료를 종료할 수 있습니다.`, `이미 종료된 진료입니다.` 및 공통 인증 오류.
...
HTTP 400: 잘못된 사용자 유형, 채팅방 없음, 기관 소유권 없음, 종료된 진료, `녹음 파일이 필요합니다.`, `음성 변환 결과가 비어 있습니다.`, `Record와 Archive 연결에 실패했습니다.`. 외부 음성 인식·파일 IO 예외는 전용 핸들러가 없어 확인 필요다.
...
`data { archiveId, title, archiveDate, text, allChatText }`.
...
33번과 같은 `FindAllArchiveResponse` 구조다.
