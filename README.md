...
| 서비스 현황 분석 기반 | 신규 피보호자의 질병 정보 저장 구조 및 세 사용자 유형의 마지막 로그인 시각 관리 |
...
- `WardUserDisease`는 피보호자(`WardUser`)와 일대일로 연결되는 질병 정보 엔티티입니다. 신규 피보호자 생성 시 함께 생성하고, `CascadeType.ALL`로 저장을 전파합니다.
- 현재 질병 횟수 필드는 `coldCount`, `coldCount2`~`coldCount10`이며 초기값은 모두 `0`입니다. 이번 변경은 데이터 저장 구조를 마련하는 단계로, 진료 결과에 따른 횟수 증가·질병 분류·통계 조회 기능은 포함하지 않습니다.
...
- 기존에 가입한 피보호자의 질병 정보 생성은 추후 별도 처리합니다.
