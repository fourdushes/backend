...
| 이름 | 역할 |
|---|---|
| 양준형 | 서비스 기획, ERD·아키텍처 설계, 사용자·기관·보호 관계·진료 요청·기록·인증 등 백엔드 전반 구현, 생성형 AI를 활용한 프론트엔드 구현 및 API 연동 |
| 강윤모 | 클라우드 인프라 구축 및 운영, 모델 서버 연동, 문의하기 기능 구현 |
| 임세훈 | 마케팅, 재무, 생성형 AI를 활용한 프론트엔드 구현 |
| 송도훈 | AI 상담 내용 정리 및 요약 기능, 텍스트 정제 및 성능 최적화 |
...
- 피보호자·보호자·기관 소속 사용자는 가입 시 `lastLoginDateTime`을 초기화하고, 로그인 검증을 통과한 뒤 현재 시각으로 갱신합니다. 로그인 트랜잭션이 정상 완료되면 DB에 반영되며, 전체 접속 이력이 아닌 마지막 로그인 시각 한 건을 보관합니다.
- 기존 아카이브에 질병 기록이 없으면 분류 갱신 시 생성합니다. 과거 기록의 일괄 재분류나 기존 DB 테이블 삭제는 자동 수행하지 않습니다.
...
진료 종료 API는 시스템 메시지를 제외한 대화를 시간순으로 합쳐 AI Summary API에 전달합니다. 응답의 `mainSymptoms`, `doctorOpinion`, `remember`, `questionAnswer`, `difficultWords`는 아카이브 요약으로 저장하고, `disease`는 아카이브별 질병 분류와 AI 원문으로 저장합니다. 일부 요약 필드만 비어 있으면 해당 값을 `"없음"`으로 저장하고, 전체 요약이 비어 있거나 외부 서비스 호출이 실패하면 HTTP `502`를 반환합니다.
